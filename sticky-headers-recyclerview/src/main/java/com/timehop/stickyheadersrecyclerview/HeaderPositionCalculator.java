...
public class HeaderPositionCalculator {
    ...
    private void initDefaultHeaderOffset(Rect headerMargins, RecyclerView recyclerView, View header, View firstView, int orientation) {
        ...
        if (orientation == LinearLayoutManager.VERTICAL) {
            ...
            if (mAdapter.isHeaderInLeft()) {
                translationY = Math.max(
                        firstView.getTop() - topMargin - mTempRect1.bottom,
            ...
            if (mAdapter.isHeaderInLeft()) {
                translationX = Math.max(
                        firstView.getLeft() - leftMargin - mTempRect1.right,
    ...
    private boolean isStickyHeaderBeingPushedOffscreen(RecyclerView recyclerView, View stickyHeader) {
        ...
        if (firstViewUnderHeaderPosition > 0 && hasNewHeader(firstViewUnderHeaderPosition, isReverseLayout)) {
            ...
            if (mOrientationProvider.getOrientation(recyclerView) == LinearLayoutManager.VERTICAL) {
                int topOfNextHeader = viewAfterHeader.getTop() - mTempRect1.bottom - (mAdapter.isHeaderInLeft() ? 0 : nextHeader.getHeight()) - mTempRect1.top;
                ...
                int leftOfNextHeader = viewAfterHeader.getLeft() - mTempRect1.right - (mAdapter.isHeaderInLeft() ? 0 : nextHeader.getWidth()) - mTempRect1.left;
    ...
    private void translateHeaderWithNextHeader(RecyclerView recyclerView, int orientation, Rect translation,
        ...
        if (orientation == LinearLayoutManager.VERTICAL) {
            ...
            int shiftFromNextHeader = viewAfterNextHeader.getTop() - (mAdapter.isHeaderInLeft() ? 0 : nextHeader.getHeight()) - mTempRect1.bottom - mTempRect1.top - currentHeader.getHeight() - topOfStickyHeader;
            ...
            int shiftFromNextHeader = viewAfterNextHeader.getLeft() - (mAdapter.isHeaderInLeft() ? 0 : nextHeader.getWidth()) - mTempRect1.right - mTempRect1.left - currentHeader.getWidth() - leftOfStickyHeader;
